...
void casosSegunProvincias(int n, vector<vector<string>> *tabla) {//ANDA BIEN
...
void fallecidos_provincias(int n, vector<vector<string>> *tabla) {//ANDA BIEN
    vector<string> provincias;
    provincias.clear();
...
void casos_edad(int n, vector<vector<string>> *tabla, int anios) {
    vector<string> edad;
    vector<string> provincias;
    provincias.clear();
    edad.clear();
    ...
    for (int i = 1; i < (*tabla).size(); i++) {
        if ((*tabla)[i][1] != "null") {
            if (provincias.empty()) {
                ...
                ban = false; //Sino seteamos la variable ban como falsa
                for (int j = 0; j < provincias.size(); j++) {//Recorremos el vector
                    if (provincias[j] == (*tabla)[i][1]) { //Comparamos los valores de la tabla original y el vector
                        ban = true;
                    }
                }
                if (!ban)//Si el booleano es verdadero porque coincidieron los datos
                    provincias.push_back((*tabla)[i][1]);//Agregamos los datos de las ids provincias
            }
        }
    }
    ...
    for (int i = 1; i < (*tabla).size(); i++) {
        if ((*tabla)[i][0] != "null") {
            if (edad.empty()) {
                ...
                ban2 = false; //Sino seteamos la variable ban coo falsa
                for (int j = 0; j < edad.size(); j++) {//Recorremos el vector
                    if (edad[j] == (*tabla)[i][0]) { //y comparamos los valores de la tabla original y el vector
                        ban2 = true;
                    }
                }
                if (!ban2)//Si el booleano es verdadero porque coincidieron los datos
                    edad.push_back((*tabla)[i][0]);//Agregamos los datos de las ids provincias
            }
        }
    }
    ...
    for (int i = 1; i < (*tabla).size(); i++) {
        if ((*tabla)[i][3] != "null") {
            if (fecha.empty()) {
                ...
                ban = false; //Sino seteamos la variable ban coo falsa
                for (int j = 0; j < fecha.size(); j++) {//Recorremos el vector
                    if (fecha[j] == (*tabla)[i][3]) { //Comparamos los valores de la tabla original y el vector
                        ban = true;
                    }
                }
                if (!ban)//Si el booleano es verdadero porque coincidieron los datos
                    fecha.push_back((*tabla)[i][3]);//Agregamos los datos de las ids provincias
            }
        }
    }
